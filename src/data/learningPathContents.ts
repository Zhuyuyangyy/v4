export type StageContent = { title: string; type: string; isRemedial?: boolean }
...
export const NODE_CONTENT: Record<string, StageContent[][]> = {
...
export function getLearningPathContent(topicId: string): StageContent[][] | null {
  return NODE_CONTENT[topicId] || null
}

export function getStageLabel(index: number): string {
  return STAGE_META[index]?.label ?? ''
}
