    ...
    <div class="max-w-4xl mx-auto w-full space-y-6 text-left">
      <div class="border-b border-slate-900 pb-4 shrink-0">
        <h1 class="font-extrabold text-white text-xl tracking-tight font-display flex items-center gap-2">
          <HistoryIcon class="w-6 h-6 text-blue-300" />
      ...
      <div class="space-y-4">
        <div v-for="hist in historyPresets" :key="hist.id"
          ...
          <div class="space-y-3 flex-1 text-left p-4 md:p-0 md:pl-4">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs font-bold px-3 py-1 rounded-full bg-slate-900/40 backdrop-blur-md text-blue-300 border border-slate-800/40">{{ hist.evaluation }}</span>
            </div>
            <div>
          ...
          <div class="flex md:flex-col items-end justify-between md:justify-center border-t md:border-t-0 border-slate-900 pt-3.5 md:pt-0 w-full md:w-auto shrink-0 gap-4 p-4 md:p-0 md:pr-4">
            <div class="text-left md:text-right">
              ...
              <span class="text-3xl font-black text-blue-300 font-display">{{ hist.score }}</span>
            </div>
        ...
        <div class="bg-transparent rounded-2xl p-6 border border-slate-900 text-slate-300 flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
          <div class="space-y-1">
            <h4 class="font-bold text-sm text-blue-300 flex items-center gap-1">
          ...
          <div class="flex gap-2 shrink-0">
            <span class="bg-blue-500/10 text-blue-400 border border-blue-500/20 text-xs font-extrabold px-3 py-1.5 rounded-full">🎖️ 领域新星</span>
            <span class="bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 text-xs font-extrabold px-3 py-1.5 rounded-full">🚀 极速破壁人</span>
          </div>
        </div>
      </div>
    </div>
