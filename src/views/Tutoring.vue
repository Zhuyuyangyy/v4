...
const sessionHistory = [
  { title: '鏈哄櫒瀛︿範棰勪範', count: 6, date: '浠婂ぉ', scenario: 'preview' as StudyScenario },
  { title: 'KNN 绠楁硶璋冭瘯', count: 12, date: '鏄ㄥぉ', scenario: 'homework' as StudyScenario },
  { title: '鏈熸湯鑰冭瘯鍐插埡', count: 9, date: '3澶╁墠', scenario: 'exam' as StudyScenario },
]
...
<template>
  <div class="tutor">
    <div class="tutor-hero">
      <div>
        <div class="hero-badge">鏅鸿兘杈呭</div>
        <h1 class="hero-title">
          <span class="gradient-text">{{ activeSubModeConfig?.label ?? '鍗虫椂绛旂枒' }}</span>
        </h1>
        <p class="hero-desc">{{ activeScenarioConfig?.description ?? '澶氭ā寮忔繁搴﹁瑙ｏ紝闅忔椂闅忓湴瑙ｅ喅浣犵殑瀛︿範闂' }}</p>
      </div>
      <div class="hero-actions">
        <button class="hero-btn course-btn" @click="showCoursePanel = !showCoursePanel">
          <BookOpen :size="15" stroke-width="1.5" />
          <span>{{ selectedCourse?.name ?? '閫夋嫨璇剧▼' }}</span>
    ...
    <transition name="slide-up">
      <div v-if="showCoursePanel" class="course-panel">
        ...
        >
          ...
          <span class="cc-diff">{{ c.difficulty === 'beginner' ? '鍏ラ棬' : c.difficulty === 'intermediate' ? '杩涢樁' : '楂樼骇' }}</span>
        </button>
      </div>
    </transition>
      ...
      <div v-if="showHistory" class="history-panel">
        <button v-for="s in sessionHistory" :key="s.title" class="history-item" @click="selectSession(s)">
          ...
          <div class="history-info">
            ...
            <span class="history-meta">{{ s.count }} 鏉″璇?路 {{ s.date }}</span>
          </div>
    ...
    <div class="tutor-body">
      <div class="tutor-main">
        <div v-if="history.length > 0" class="conversation">
          ...
          <div v-if="isAsking" class="qa-pair pending-pair">
            <div class="answer-bubble pending-bubble">
              <div class="bubble-avatar a-avatar">A</div>
              <div class="bubble-content">
                <div class="answer-body pending-answer">姝ｅ湪鎬濊�冧腑...</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="input-section">
          <div class="mode-indicator">
            {{ activeScenarioConfig?.label ?? '杈呭' }} 路 {{ activeSubModeConfig?.label ?? '闂瓟' }}
          </div>
          <div class="input-row">
            <input
              v-model="question"
          ...
          <div class="modal-panel">
            ...
            <div class="mp-body">
              <FlowChart
                ...
                :title="selectedCourse?.name ? `${selectedCourse.name} 娴佺▼` : '鏁版嵁澶勭悊娴佺▼'"
              />
              <CodeCanvas
                v-if="activeModalPanel === 'code'"
                :examples="currentCodeExamples"
                language="python"
                @explain="code => { question = '璇疯В閲婅繖娈典唬鐮侊細' + code.slice(0, 100); activeModalPanel = null }"
              />
              <MindMap
                v-if="activeModalPanel === 'mindmap'"
                :nodes="currentMindMap ?? []"
                :title="selectedCourse?.name ?? '鐭ヨ瘑浣撶郴'"
                :color="selectedCourse?.color ?? '#00d4ff'"
                @node-click="node => { question = '解释一下' + node.label; activeModalPanel = null }"
              />
            </div>
          </div>
        </div>
      ...
      <div class="tutor-sidebar">
        <button class="dh-toggle" :class="{ active: true }" title="AI 数字人">
