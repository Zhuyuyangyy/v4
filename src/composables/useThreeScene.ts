...
export function useThreeScene(container: HTMLElement) {
  ...
  const bloomPass = new UnrealBloomPass(
    ...
    0.4,   // strength
    0.3,   // radius
    0.7,   // threshold (higher — only bright things bloom)
  )
  ...
  const sunLight = new THREE.PointLight(0xffeedd, 60, 100, 0.5)
  ...
  function animate() {
    ...
    // Pulsing point light
    sunLight.intensity = 40 + Math.sin(time * 2.0) * 6 + Math.sin(time * 4.5) * 3
